  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        opacity: [0.72, 1, 0.72],
        boxShadow: [
          '0 6px 14px rgba(184,148,91,0.18)',
          '0 8px 22px rgba(184,148,91,0.28)',
          '0 6px 14px rgba(184,148,91,0.18)',
        ],
      }}
...
export default function PortfolioCategoryPage() {
  ...
  const renderStoryPhotoCard = (photo, photoIndex, suffix = '') => (
    <motion.article
      key={`${photo.id}-${photoIndex}`}
      initial={{ opacity: 0, y: 24 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.45, delay: Math.min(photoIndex * 0.06, 0.24) }}
      className="group mb-5 break-inside-avoid overflow-hidden rounded-[16px]"
    >
      <Link to={`${category.route}/${photo.id}${suffix}`} className="block">
  ...
  const displayActiveTitle = photoId
    ? (isHyphenTitleCategory && activeTitle
      ...
      <div className="mx-auto max-w-7xl px-6 pb-14 pt-[6.8rem] sm:pb-16 sm:pt-[7.3rem] lg:pb-20 lg:pt-[7.8rem]">
        ...
        >
          ...
          <h1 className="mt-3 font-heading text-[2.45rem] leading-[0.98] tracking-[-0.025em] text-[#2d211a] sm:text-[3.8rem]">
            {photoId ? displayActiveTitle : <TitleWithAmpersand title={category.title} />}
          </h1>
                ...
                {weddingSections.map((section, index) => (
                  section.photos.length ? (
                    ...
                    >
                      ...
                      <div className="columns-1 gap-5 space-y-5 sm:columns-2 xl:columns-3">
                        {section.photos.map((photo, photoIndex) => (
                          <div key={`${section.value}-${photo.id}-${photoIndex}`}>
                            {renderStoryPhotoCard(photo, photoIndex, `?type=${section.value}`)}
                          </div>
                        ))}
                      </div>
                    </motion.div>
                    ...
                    >
                      ...
                      <p className="max-w-xl font-source-serif text-[1.18rem] leading-7 tracking-[0.01em] text-[#7f7265] sm:text-[1.24rem]">{section.subtitle}</p>
